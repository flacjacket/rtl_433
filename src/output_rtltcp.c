...
typedef struct rtltcp_server {
    ...
    int data_inuse;  ///< the data slot that is in use, -1 otherwise
    void *data_buf[DATA_SLOTS];
    int data_size[DATA_SLOTS];
    int data_len[DATA_SLOTS];
...
static int parse_command(r_cfg_t *cfg, uint8_t const *buf, int len)
{
    ...
    case RTLTCP_SET_FREQ:
        fprintf(stderr, "Received rtl_tcp command SET_FREQ with %u\n", arg);
        // set_center_freq(cfg, arg);
        break;
    case RTLTCP_SET_SAMPLE_RATE:
        fprintf(stderr, "Received rtl_tcp command SET_SAMPLE_RATE with %u\n", arg);
        // set_sample_rate(cfg, arg);
        break;
    case RTLTCP_SET_GAIN_MODE:
        fprintf(stderr, "Received rtl_tcp command SET_GAIN_MODE with %u\n", arg);
        break;
    case RTLTCP_SET_GAIN:
        fprintf(stderr, "Received rtl_tcp command SET_GAIN with %u\n", arg);
        break;
    case RTLTCP_SET_FREQ_CORRECTION:
        fprintf(stderr, "Received rtl_tcp command SET_FREQ_CORRECTION with %u\n", arg);
        // set_freq_correction(cfg, (int)arg);
        break;
    case RTLTCP_SET_IF_TUNER_GAIN:
        fprintf(stderr, "Received rtl_tcp command SET_IF_TUNER_GAIN with %u\n", arg);
        break;
    case RTLTCP_SET_TEST_MODE:
        fprintf(stderr, "Received rtl_tcp command SET_TEST_MODE with %u\n", arg);
        break;
    case RTLTCP_SET_AGC_MODE:
        fprintf(stderr, "Received rtl_tcp command SET_AGC_MODE with %u\n", arg);
        break;
    case RTLTCP_SET_DIRECT_SAMPLING:
        fprintf(stderr, "Received rtl_tcp command SET_DIRECT_SAMPLING with %u\n", arg);
        break;
    case RTLTCP_SET_OFFSET_TUNING:
        fprintf(stderr, "Received rtl_tcp command SET_OFFSET_TUNING with %u\n", arg);
        break;
    case RTLTCP_SET_RTL_XTAL:
        fprintf(stderr, "Received rtl_tcp command SET_RTL_XTAL with %u\n", arg);
        break;
    case RTLTCP_SET_TUNER_XTAL:
        fprintf(stderr, "Received rtl_tcp command SET_TUNER_XTAL with %u\n", arg);
        break;
    case RTLTCP_SET_TUNER_GAIN_BY_ID:
        fprintf(stderr, "Received rtl_tcp command SET_TUNER_GAIN_BY_ID with %u\n", arg);
        break;
    case RTLTCP_SET_BIAS_TEE:
        fprintf(stderr, "Received rtl_tcp command SET_BIAS_TEE with %u\n", arg);
        break;
    default:
        fprintf(stderr, "Received unknown rtl_tcp command %d with %u\n", cmd, arg);
        break;
    }
...
static void rtltcp_broadcast_send(rtltcp_server_t *srv, uint8_t const *data, int len)
{
    ...
    if (srv->data_buf[slot] == NULL || srv->data_size[slot] < len) {
        fprintf(stderr, "%s: allocating buffer for rtl_tcp\n", __func__);
...
static THREAD_RETURN THREAD_CALL accept_thread(void *arg)
{
    ...
    for (;;) {
        ...
        printf("rtl_tcp client connected from %s port %s\n", host, port);
        ...
        printf("rtl_tcp client disconnected from %s port %s\n", host, port);
        close(sock);
    }
    fprintf(stderr, "accept_thread exit...\n");
    return NULL;
}
...
static int rtltcp_server_start(rtltcp_server_t *srv, char const *host, char const *port, r_cfg_t *cfg, struct raw_output *output)
{
    ...
    printf("Starting rtl_tcp server on address %s %s\n", address, portstr);
...
static int rtltcp_server_stop(rtltcp_server_t *srv)
{
    ...
    fprintf(stderr, "%s: stopping rtl_tcp thread...\n", __func__);
